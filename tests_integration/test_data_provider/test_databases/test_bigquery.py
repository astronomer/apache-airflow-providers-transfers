...
def test_table_exists_raises_exception():
    ...
    database = BigqueryDataProvider(
        Table(name="nonexistent-table", metadata=Metadata(schema=BIGQUERY_SCHEMA), conn_id=CUSTOM_CONN_ID),
        transfer_mode=TransferMode.NONNATIVE,
    )
    table = Table(name="non-existent-table", metadata=Metadata(schema=BIGQUERY_SCHEMA))
...
def test_s3_to_bigquery_native_path(sample_dag, src_dataset_fixture):
    ...
    with sample_dag:
        # [START transfer_from_s3_to_bigquery_natively]
    ...
    # [END transfer_from_s3_to_bigquery_natively]
    run_dag(sample_dag)
    file_dataframe = export_to_dataframe(file_dataprovider)
    table_dataframe = export_to_dataframe(file_dataprovider)
...
def test_gcs_to_bigquery_native_path(sample_dag, src_dataset_fixture):
    ...
    with sample_dag:
        # [START transfer_from_gcs_to_bigquery_natively]
        UniversalTransferOperator(
        ...
        # [END transfer_from_gcs_to_bigquery_natively]
    run_dag(sample_dag)
    file_dataframe = export_to_dataframe(file_dataprovider)
